...
1. **Capture the idea.** Ask the user to describe what they want to build in one or two
   sentences. Then ask at most 3–5 focused follow-up questions (who is it for, what's the
   one thing it must do first, any hard constraints).
<!-- BEGIN:memory -->
   Write the answers into `memory/project-brief.md` — briefly; it can be polished later.
<!-- END:memory -->
<!-- BEGIN:scrum -->
...
If the workspace is not fresh (the brief is filled, sprints exist), say so and point the
