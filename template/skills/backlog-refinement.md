---
description: Refine the product backlog — split, clarify, estimate and reorder items with the PO.
---
...
2. For each item near the top (aim for 1–2 sprints of `refined` work, no more):
   ...
   - Flag technical risks or dependencies; propose a spike item if a question blocks
     estimation.
3. Where the current order looks wrong (low value above high value, dependency
   inversions), propose a reordering — but let the PO decide. Ask, don't assume.
4. Move items that met their criteria to `refined`; park vague far-future ideas in the
   Icebox.
5. Summarize: what changed, what's now ready for planning, what needs a PO decision.
